...
public class ReporterTxt extends AbstractReport {
    ...
    protected void dump(String header, List<Resume> list) {
        ...
        String pattern = "\t%10s %10s | %10s | %7s | %-24s | %10s | %10s\n";
        System.out.printf(pattern, "", "#", "TIME (ms)", "%", " ON", "STATUS", "INPUT <-> OUTPUT");
        pattern = "\t%10s %10s | %10s | %7.2f | %23s  | %10s | %10s\n";
        for (Resume r : list) {
            System.out.printf(pattern, "", r.getIndex(), r.getTime(), asPercentage(r.getTime()), r.getTimestamp(), r.getStatus().getName(), r.getInput() + " <-> " + r.getOutput());
        }
        System.out.println("\t          ----------------------------------");
        pattern = "\t%10s %10s : %10d\n";
